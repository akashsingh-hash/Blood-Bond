...
const RequestForm = ({ hospitalId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/hospitals/${hospitalId}/request-blood`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
      ...
      toast.success('Request sent successfully! Hospital will contact you within 30 minutes.');
